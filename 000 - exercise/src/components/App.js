...
1. Let's implement a simple bank account! It's similar to the example that I used as an analogy to 
explain how useReducer works, but it's simplified (we're not using account numbers here)
...
4. When the account is opened, isActive is set to true. There is also a minimum deposit amount of 500 to open an account 
(which means that the balance will start at 500)

5. Customer can only request a loan if there is no loan yet. If that condition is met, the requested amount will be registered 
in the 'loan' state, and it will be added to the balance. If the condition is not met, just return the current state
  ...
  switch (action.type) {
    ...
    case "withdraw":
      return {
        ...state,
        balance:
          state.balance - action.payload >= 0 ? state.balance - action.payload : state.balance,
      };
    case "requestLoan":
      return {
        ...state,
...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <p className="text-el">Balance: {balance}</p>
      <p className="text-el">Loan: {loan}</p>
